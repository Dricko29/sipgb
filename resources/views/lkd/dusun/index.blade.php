        ...
        <div class="col-lg-12">
            ...
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title mb-0">Dusun</h4>
                </div><!-- end card header -->
                ...
                <div class="card-body">
                    ...
                    <div class="row g-4 mb-3">
                        <div class="col-sm-auto">
                            <div>
                                <a href="{{ route('dusun.create') }}" class="btn btn-sm btn-success add-btn"><i class="ri-add-line align-bottom me-1"></i> Tambah</a>
                            </div>
                        </div>
                    </div>
                    @endcan
                            ...
                            <table id="example" class="table align-middle dt-responsive table-nowrap" style="width:100%">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{ $item->nama }}</td>
                                        <td>{{ $item->nama_kadus }}</td>
                                        <td>{{ $item->nik_kadus }}</td>
                                        <td>{{ $item->jk->nama }}</td>
                                        <td>{{ $item->kontak }}</td>
                                        <td>{{ $item->alamat }}</td>
                                        <td>
                                            <a href="{{ route('dusun.show', $item) }}" class="btn btn-sm btn-success">lihat</a>
                                            @can('admin_akses')
                                            <a href="{{ route('dusun.edit', $item) }}" class="btn btn-sm btn-warning">Edit</a>
                                            <form action="{{ route('dusun.destroy', $item) }}" method="POST" onsubmit="return confirm('anda yakin ?')" class="d-inline">
                                                @csrf
                                                @method('DELETE')
